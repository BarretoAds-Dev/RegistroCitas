	...
	try {
		...
		for (let dayOffset = 1; dayOffset <= days; dayOffset++) {
			...
			for (let hour = startHour; hour <= endHour; hour++) {
				...
				const endTime = `${String(endHourCalc).padStart(2, '0')}:${String(endMinCalc).padStart(2, '0')}:00`;
		...
		if (checkError) {
			return new Response(
				JSON.stringify({ error: 'Error al verificar slots existentes', details: checkError.message }),
				{ status: 500, headers: { 'Content-Type': 'application/json' } }
			);
		}
		...
		const newSlots = slots.filter(s => !existingKeys.has(`${s.date}_${s.start_time}`));
		...
		if (insertError) {
			return new Response(
				JSON.stringify({ error: 'Error al insertar slots', details: insertError.message }),
				{ status: 500, headers: { 'Content-Type': 'application/json' } }
			);
		}
		...
		return new Response(
			JSON.stringify({
				message: `Se generaron ${inserted?.length || 0} slots nuevos`,
